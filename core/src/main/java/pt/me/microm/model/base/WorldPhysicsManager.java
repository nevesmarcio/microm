...
//		setPauseSim(true);
    ...
    void listenModels(AbstractModelEvent abstractModelEvent) {
        ...
        // This should solve the startup crashes, as every object added is handled after the gameTick
        add(new ICommand() {
            @Override
            public Object handler(Object... a) {

                Body tmpBody;
                if (abstractModelEvent.getEventType() == AbstractModelEvent.OnCreate.class) {
                    if (abstractModelEvent.getEventSource() instanceof IBodyStatic) {
                        ...
                        logger.info("IBodyStatic found: {}", abstractModelEvent.getEventSource());
                    }
                    if (abstractModelEvent.getEventSource() instanceof IBodyDynamic) {
                        ...
                        logger.info("IBodyDynamic found: {}", abstractModelEvent.getEventSource());
                    }
                }
...
//		daBoxShape.setRadius(newRadius);
    ...
    public void update(GameTickEvent e) {
        ...
        if (logger.isDebugEnabled())
            logger.debug("[WorldModel timestep]: time elapsed=" + elapsedNanoTime / GAME_CONSTANTS.ONE_SECOND_TO_NANO + "s");
...
//			getPhysicsWorld().step(elapsedNanoTime/(float)GAME_CONSTANTS.ONE_SECOND_TO_NANO, 8, 3);
        ...
        if (logger.isDebugEnabled())
            logger.debug("[physics-step]: step=" + elapsedNanoTime / (float) GAME_CONSTANTS.ONE_SECOND_TO_NANO);
//		}		
