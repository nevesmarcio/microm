...
public class MyContactListener extends EventDispatcher implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        if (logger.isDebugEnabled())
            logger.debug("[default-contact-handler] : BeginContact => (" + contact.getChildIndexA() + "," + contact.getChildIndexB() + ") => " + contact.getFixtureA().toString() + " :: " + contact.getFixtureB());
    ...
    public void endContact(Contact contact) {
        ...
        // Cada model deverá fazer o override a este método para tratar dos contactos
        if (logger.isDebugEnabled())
            logger.debug("[default-contact-handler] : EndContact => (" + contact.getChildIndexA() + "," + contact.getChildIndexB() + ") => " + contact.getFixtureA().toString() + " :: " + contact.getFixtureB());
        ...
        if (a == 0) {
            ...
            if (logger.isDebugEnabled())
                logger.debug((IContact) contact.getFixtureA().getBody().getUserData() + " -o- " + (IBody) contact.getFixtureB().getBody().getUserData());
        }
