...
//		camera.update();
		...
		Vector3 camCenter = new Vector3(0.0f, real_h/2, 0.0f);
		float camRadius = (float)( (real_h/2f) / Math.tan(Math.toRadians(fovy/2f)));  // tendo em conta o fov, a que distância está a camara do "near clipping plan" - viewport	
		...
		Gdx.input.setInputProcessor(new InputAdapter() {
			@Override public boolean touchDown(int x, int y, int pointer, int button) {
				...
				camera.unproject(vec.set(x,y,0.01f));
				
				bottleModel.setTransform(vec.x, vec.y, 0f);
